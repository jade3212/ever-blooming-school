...
function GalleryContent({ images }: { images: GalleryItem[] }) {
  const searchParams = useSearchParams();
  const router = useRouter();

  const selectedId = searchParams.get("image");
  const selected = selectedId
    ? images.find((img) => img.id === Number(selectedId))
    : null;
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
        {visible.map((img) => (
          <button
            key={img.id}
            onClick={() =>
              router.push(`/gallery?image=${img.id}`, { scroll: false })
            }
      ...
      {selected && (
        ...
        >
          ...
          >
            <button
              onClick={closeModal}
              className="absolute right-4 top-4 z-10 flex h-10 w-10 items-center justify-center rounded-full bg-white/90 text-slate-700 shadow-lg hover:bg-white"
